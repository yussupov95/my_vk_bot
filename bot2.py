...
# Вставь сюда свой полный токен (в одну строку)
TOKEN = os.getenv("VK_BOT_TOKEN")="vk1.a.IShdbvc7y-WNl-laMuw1g-vYEwLHjNk-nPqHZSsPbjC0Ul-dYBVjPyeur0z1i4L5r-XARvPy3p38cedqN38bFvUKqM-uRf8F8AOlJcsqe5r30NWWxep87JyZOw8xwLXXjtr5VDkrm34oo8Doznrqh3K-CdPhUd4ymOI-sjYh47PC4gisZckSK8SOFG-7nzxyBofyRfk9PUm5yaFsWVRFsQ"  # Используйте переменные окружения для токена
...
async def donate_handler(message: Message):
    ...
    await message.answer(
[12.03.2026 1:06] ChatGPT 5 | Gemini 3 | Nano Banana:         f"🙏 Спасибо за поддержку!\n\n"
        f"💳 Номер карты Сбера:\n{card_number}\n\n"
        f"Любая сумма поможет развитию бота.",
        keyboard=get_keyboard()
    )
...
async def photo_handler(message: Message):
    ...
    # Сохраните ссылку в истории
    add_link(message.from_id, short_url, "photo")
    
    await message.answer(f"Вот ваша короткая ссылка: {short_url}", keyboard=get_keyboard())
...
async def video_handler(message: Message):
    ...
    video = message.attachments[0].video
    long_url = video.player
    short_url = await shorten_url(long_url)
    
    # Сохраните ссылку в истории
    add_link(message.from_id, short_url, "video")
